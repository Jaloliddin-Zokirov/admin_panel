...
const EditUser = () => {
  ...
  useEffect(() => {
    if (userCard[lang] === undefined) {
      navigate(`/user`);
    } else {
      setFirstname(userCard[lang]?.firstname);
      setLastname(userCard[lang]?.lastname);
    }
  }, [userCard, lang]);
  ...
  const handleSubmit = async (evt) => {
    ...
    message.info(lang === "ru" ? "В развитие" : "Ishlab chiqish jarayonida");
    ...
    //   const response = await axios.put(`/api/audios/${userCard.id}`, formData, {
    //     headers: {
  ...
  return (
    ...
    >
      <h2 className={`${styles.title} ${styles.post__title}`}>
        ...
        {lang === "ru"
          ? `Редактировать: ${firstname} ${lastname}`
          : `Tahrirlash: ${firstname} ${lastname}`}
      </h2>
